...
public class NeoSwoop : Card, IRegisterable, IHasCustomCardTraits
{
    public static Spr TempPlaceHolderArt;
    
    private static string _cardName = "Swoop";
    ...
    {
        _descriptionNone =
            $"Gain {_actionOneAmountNone} <c=status>evade</c>, <c=downside>but draw {_actionTwoAmountNone}</c> less cards next turn.";
        _descriptionA =
            $"Gain {_actionOneAmountA} <c=status>evade</c>.";
        _descriptionB =
            $"Gain {_actionOneAmountB} <c=status>evade</c>, <c=downside>but draw {_actionTwoAmountB}</c> less cards next turn.";
        if (_artmode == 0)
        {
            if (upgrade == Upgrade.None)
            {
