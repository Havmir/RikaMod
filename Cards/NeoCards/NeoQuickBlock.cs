...
public class NeoQuickBlock : Card, IRegisterable, IHasCustomCardTraits
{
    public static Spr TempPlaceHolderArt;
    
    private static string _cardName = "QuickBlock";
    // Make sure to update rarirty
    private int _cardcostNone = 0;
    private int _cardcostA = 0;
    private int _cardcostB = 1;
    private string _actionOneNone = "status.tempShield";
    private string _actionOneA = "status.tempShield";
    private string _actionOneB = "status.tempShield";
    private string _actionTwoB = "status.shield";
    ...
    private Spr? _artNone = StableSpr.cards_Shield;
    private Spr? _artA =  StableSpr.cards_Shield;
    private Spr? _artB =  StableSpr.cards_Shield;
    ...
    {
        _descriptionNone =
            $"On draw, <c=downside>-{_cardcostNone} energy</c> but gain {_actionOneAmountNone} <c=status>temp shield</c>.";
        _descriptionA =
            $"On draw, <c=downside>-{_cardcostA} energy</c> but gain {_actionOneAmountA} <c=status>temp shield</c>.";
        _descriptionB =
            $"On draw, <c=downside>-{_cardcostB} energy</c>, gain {_actionOneAmountB} <c=status>temp shield</c> & {_actionTwoAmountB} <c=status>shield</c>.";
        if (_artmode == 0)
        {
            if (upgrade == Upgrade.None)
            {
