...
public class NeoKiteing: Card, IRegisterable, IHasCustomCardTraits
{
    private static string _kiteingStatusName = ArtManager.KiteingDisplayName;
    
    public static Spr TempPlaceHolderArt;
    public static Spr KiteingCardBackgroundSprite;
    
    private static string _cardName = "Kiteing";
    // Make sure to update rarirty
    private int _cardcostNone = 2;
    private int _cardcostA = 1;
    private int _cardcostB = 2;
    private string _actionOneNone = _kiteingStatusName;
    private string _actionOneA = _kiteingStatusName;
    private string _actionOneB = _kiteingStatusName;
    ...
    // private Spr? _artNone = KiteingCardBackgroundSprite; // This leads to a crash if left uncommented for some odd reason. You'll just have to manually put in the art ~ Havmir 1/1/2026
    // private Spr? _artA =  KiteingCardBackgroundSprite; 
    // private Spr? _artB =  KiteingCardBackgroundSprite; 
    ...
    public static void
        Register(IPluginPackage<IModManifest> package,
            IModHelper helper)
    {
        // KiteingCardBackgroundSprite = ModEntry.RegisterSprite(package, "assets/Card/KiteingCardBackground.png").Sprite;
    ...
    {
        _descriptionNone =
            $"On draw, <c=downside>-{_cardcostNone} energy</c>, but gain {_actionOneAmountNone}      <c=status>{_kiteingStatusName}</c>.";
        _descriptionA =
            $"On draw, <c=downside>-{_cardcostA} energy</c>, but gain {_actionOneAmountA}      <c=status>{_kiteingStatusName}</c>.";
        _descriptionB =
            $"On draw, <c=downside>-{_cardcostB} energy</c>, but gain {_actionOneAmountB}      <c=status>{_kiteingStatusName}</c>.";
        if (_artmode == 0)
        {
            if (upgrade == Upgrade.None)
            {
