...
public class NeoHaste : Card, IRegisterable, IHasCustomCardTraits
{
    public static Spr TempPlaceHolderArt;
    
    private static string _cardName = "Haste";
    // Make sure to update rarirty
    private int _cardcostNone = 0;
    private int _cardcostA = 0;
    private int _cardcostB = 0;
    private string _actionOneNone = "status.hermes";
    private string _actionOneA = "status.hermes";
    private string _actionTwoA = "status.evade";
    private string _actionOneB = "status.hermes";
    ...
    private Spr? _artNone = StableSpr.cards_Fleetfoot;
    private Spr? _artA =  StableSpr.cards_Fleetfoot;
    private Spr? _artB =  StableSpr.cards_Fleetfoot;
    ...
    public override void OnDraw(State s, Combat c)
    {
        ...
        if (_logALotOfThings)
        {
            ModEntry.Instance.Logger.LogInformation($"[RikaMod: Neo{_cardName}.cs] CardInvestment drawn | Upgrade: {upgrade} | Rikamissing.Status = {s.ship.Get(ModEntry.Instance.Rikamissing.Status)} | Turn: {c.turn}");
        }
    }
    ...
    {
        _descriptionNone =
            $"On draw, gain {_actionOneAmountNone} <c=status>Hermes Boots</c>.";
        _descriptionA =
            $"On draw, gain {_actionOneAmountA} <c=status>draw next turn</c>.";
        _descriptionB =
            $"On draw, gain {_actionOneAmountB} <c=status>draw next turn</c>.";
        if (_artmode == 0)
        {
            if (upgrade == Upgrade.None)
            {
