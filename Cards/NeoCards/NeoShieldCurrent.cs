...
public class NeoShieldCurrent: Card, IRegisterable, IHasCustomCardTraits
{
    public static Spr TempPlaceHolderArt;
    
    private static string _cardName = "ShieldCurrent";
    // Make sure to update rarirty
    private int _cardcostNone = 1;
    private int _cardcostA = 0;
    private int _cardcostB = 1;
    private string _actionOneNone = "RikaFlux";
    private string _actionOneA = "RikaFlux";
    private string _actionOneB = "status.libra";
    ...
    private Spr? _artNone = StableSpr.cards_Flux;
    private Spr? _artA =  StableSpr.cards_Flux;
    private Spr? _artB =  StableSpr.cards_Flux;
    ...
    {
        _descriptionNone =
            $"On draw, <c=downside>-{_cardcostNone} energy</c> but gain {_actionOneAmountNone} <c=status>shield flow</c>.";
        _descriptionA =
            $"On draw, but gain {_actionOneAmountA} <c=status>shield flow</c>.";
        _descriptionB =
            $"On draw, <c=downside>-{_cardcostB} energy</c>, but gain {_actionOneAmountB} <c=status>flux</c>.";
        if (_artmode == 0)
        {
            if (upgrade == Upgrade.None)
            {
